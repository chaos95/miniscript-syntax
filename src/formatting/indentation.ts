...
const matchingClosingBracket: Record<string, string | undefined> = {
  '{': '}',
  '[': ']',
  '(': ')',
};
const matchingOpeningBracket: Record<string, string | undefined> = {
  '}': '{',
  ']': '[',
  ')': '(',
};
 ...
 * any opening bracket
 */
function isIncomplete(code: string) {
  ...
  while (index < lines.length) {
    ...
    if (openers <= 0) {
      ...
      if (!skipLine) {
        ...
        // a = [1,
        // ]
        if (!code.match(/^([[\]{}()]+\s*)*/)) indentMultiline(lines, index, index + 1);
      }
