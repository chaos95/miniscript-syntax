...
const indentation_1 = require("../../formatting/indentation");
// import * as myExtension from '../../extension';
suite('Miniscript formatting Test Suite', () => {
    void vscode.window.showInformationMessage('Start all tests.');
    function assertFormat(ctx, code, expectedResult) {
        ctx.log(`Original code: ${code}`);
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(code), expectedResult);
    }
    const whitespaces = [' ', '\t'];
    const ops = ['+', '- ', '*', '/', '%', '^', '>', '<', '=', '==', '!=', '<=', '>='];
    ...
    const blockIndent = indentation_1.MiniscriptIndenter.config.blockIndent;
    const multilineIndent = indentation_1.MiniscriptIndenter.config.multilineIndent;
    const expressionElementArb = fc.oneof(identifierArb, fc.double(), fc.integer(), fc.lorem().map(e => `"${e}"`));
    test('Should add spaces around operators', () => {
        fc.assert(fc.property(expressionElementArb, expressionElementArb, opsArb, fc.context(), (a, b, op, ctx) => {
            assertFormat(ctx, `${a}${op}${b};`, `${a} ${op.trim()} ${b}`);
        }));
        fc.assert(fc.property(fc.array(opsArb, { minLength: 2 }), fc.context(), (ops, ctx) => {
    ...
    test('Should collapse tabs, double/triple/etc spaces into single space', () => {
        fc.assert(fc.property(fc.array(whiteOrEmptyArb), fc.context(), (spaces, ctx) => {
    ...
    test('Should remove spaces before and after brackets', () => {
        fc.assert(fc.property(fc.constantFrom(['[', ']'], ['(', ')'], ['{', '}']), whiteOrEmptyArb, whiteOrEmptyArb, whiteOrEmptyArb, fc.context(), (brackets, before, after, space, ctx) => {
    ...
    test('Should remove trailing commas', () => {
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(`a=[1,2,3,]`), `a = [1, 2, 3]`);
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(`a={"1":1,"2":2,"3":3,}`), `a = {"1": 1, "2": 2, "3": 3}`);
    });
    test('Should handle minus', () => {
        fc.assert(fc.property(expressionElementArb, expressionElementArb, fc.context(), (a, b, ctx) => {
    ...
    test('Should remove empty function parenthesis', () => {
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(`function()\nend function`), `function\nend function`);
    });
    test('Should remove redundant brackets from if', () => {
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(`if(a==b) then c;`), `if a == b then c`);
        assert.strictEqual(format_1.MiniscriptFormatter.formatCode(`if(a==b) then\nend if`), `if a == b then\nend if`);
    });
    test('Should ensure space before and after comment', () => {
        fc.assert(fc.property(whiteOrEmptyArb, whiteArb, fc.context(), (before, after, ctx) => {
            const ogString = 'func' + before + '//' + after + 'comment';
            const targetString = `func //${after}comment`;
            assertFormat(ctx, ogString, targetString);
        }));
        fc.assert(fc.property(whiteOrEmptyArb, fc.context(), (before, ctx) => {
    ...
    test('Should ensure space after closing bracket before number, string or identifier', () => {
        fc.assert(fc.property(fc.constantFrom(']', ')', '}'), whiteOrEmptyArb, expressionElementArb, fc.context(), (bracket, space, value, ctx) => {
    ...
    test('Should strip empty comments', () => {
        fc.assert(fc.property(whiteOrEmptyArb, fc.context(), (comment, ctx) => {
    ...
    suite('Indentation', () => {
        const blockArb = fc.constantFrom(['if a then', 'end if'], ['while a', 'end while'], ['for a in b', 'end for'], ['a = function', 'end function']);
        test('Should remove indentation from dangling lines', () => {
            fc.assert(fc.property(whiteOrEmptyArb, fc.context(), (indent, ctx) => {
        ...
        test('Should add indentation in blocks', () => {
            fc.assert(fc.property(blockArb, fc.context(), (block, ctx) => {
                assertFormat(ctx, `${block[0]}\nbody\n${block[1]}`, `${block[0]}\n${blockIndent}body\n${block[1]}`);
            }));
        });
        test('Should add indentation in nested blocks', () => {
            fc.assert(fc.property(fc.array(blockArb, { minLength: 2 }), fc.context(), (blocks, ctx) => {
                let ogString = '';
                let targetString = '';
        ...
        test('Should add indentation in multiline operations', () => {
            fc.assert(fc.property(opsArb, fc.context(), (op, ctx) => {
                assertFormat(ctx, `a ${op}\nb`, `a ${op.trim()}\n${multilineIndent}b`);
            }));
            fc.assert(fc.property(fc.array(fc.tuple(opsArb, identifierArb), { minLength: 2 }), fc.context(), (ops, ctx) => {
                const ogString = 'a' +
                    ops
                        .map(e => e[0] + '\n' + e[1])
                        .join('')
...
function
    doStuff
      ...
      `.trim();
            assert.strictEqual(format_1.MiniscriptFormatter.formatCode(ogString), targetString);
        });
    });
    test('Sample test', () => {
